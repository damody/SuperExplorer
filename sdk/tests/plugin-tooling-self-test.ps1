...
foreach ($required in @('Get-ConsumerTreeDigest', 'Copy-BoundedConsumerSnapshot', 'ReparsePoint', 'private no-reparse build snapshot', 'CARGO_HOME', 'CARGO_TARGET_DIR = $temporaryTarget', 'CARGO_BUILD_RUSTC', 'RUSTC_BOOTSTRAP', 'CARGO_INCREMENTAL', 'SUPEREXPLORER_TRUSTED_CARGO_SHA256', 'SUPEREXPLORER_TRUSTED_RUSTC_SHA256', '$powershellExecutable', 'Get-Process -Id $PID', '--manifest-path', 'publishedThisAttempt', 'build.complete.json', 'Repair-IncompleteBuildPublication', 'consumer_tree_sha256', '--locked --offline')) {
