...
$cargoHome = if ([string]::IsNullOrWhiteSpace($env:CARGO_HOME)) { Join-Path ([Environment]::GetFolderPath([Environment+SpecialFolder]::UserProfile)) '.cargo' } else { $env:CARGO_HOME }
...
try {
    ...
    $locationPushed = $true
    New-Item -ItemType Directory -Path $hostTarget, $pluginTarget | Out-Null
    $createdTemp += $hostTarget, $pluginTarget
    if (-not (Test-Path -LiteralPath (Join-Path $cargoHome 'registry') -PathType Container)) { throw 'Prefilled local Cargo registry cache is unavailable; bootstrap it before offline validation.' }
    ...
    $cleanupErrors = @()
    foreach ($entry in @(@($hostTarget, 'host target'), @($pluginTarget, 'plugin target'))) {
