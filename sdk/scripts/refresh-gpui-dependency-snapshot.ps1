...
try{
 ...
 Push-Location $stage
 try{
  # Resolve into an ignored local Cargo registry/cache; source trees are never
  # generated into (or promoted from) the repository.
  New-Item -ItemType Directory -Path $cargoCache -Force|Out-Null;$env:CARGO_HOME=$cargoCache;Remove-Item Env:CARGO_TARGET_DIR -ErrorAction SilentlyContinue
  ...
  # Prove all consumers resolve offline from the pre-populated local cache.
  New-Item -ItemType Directory -Path $offlineTarget -Force|Out-Null;$env:CARGO_TARGET_DIR=$offlineTarget
  foreach($manifest in @('sdk\Cargo.toml','sdk\fixtures\rust-folder-size-visual-column\Cargo.toml','sdk\fixtures\abi-root-host\Cargo.toml','sdk\fixtures\abi-root-plugin\Cargo.toml')){& cargo.exe metadata --manifest-path $manifest --locked --offline --format-version 1|Out-Null;if($LASTEXITCODE){throw "locked offline consumer metadata failed: $manifest"}}
 ...
 & git -C $RepositoryRoot add -A -- 'Cargo.lock' 'sdk/Cargo.lock' 'sdk/sdk-lock.json' 'sdk/bundle-manifest.json' 'sdk/ui-abi-fingerprint.json' 'sdk/snapshot/protected-dependency-closure.json';if($LASTEXITCODE){throw 'could not stage refreshed canonical dependency outputs'}
