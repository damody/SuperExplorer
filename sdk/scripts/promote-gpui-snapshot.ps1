...
try {
 ...
 $changed=@(Invoke-Git @('diff','--cached','--name-only'));$unexpected=@($changed|Where-Object{$_ -notin $allowed -and -not $_.StartsWith('sdk/vendor/cargo-sources/',[StringComparison]::Ordinal)});if($unexpected.Count -or @($requiredChanged|Where-Object{$_ -notin $changed}).Count){throw 'candidate patch changes an unexpected promotion surface'}
