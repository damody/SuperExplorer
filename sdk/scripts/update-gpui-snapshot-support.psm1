...
function Assert-GpuiUpdateApproval {
 param($Approval,[Parameter(Mandatory)][string]$OldRevision,[Parameter(Mandatory)][string]$NewRevision,[Parameter(Mandatory)][string]$NewTree,[Parameter(Mandatory)][string]$CandidateTreeDigest,[Parameter(Mandatory)][string]$WorkflowRunId,[Parameter(Mandatory)][string]$Nonce,[Parameter(Mandatory)][bool]$FastForward,[datetime]$Now=[DateTime]::UtcNow)
 if($FastForward -and $null -eq $Approval){return};if($null -eq $Approval){throw 'approval required'}
 $required=@('schema_version','baseline_revision','old_revision','new_revision','new_tree','candidate_tree_digest','workflow_run_id','nonce','reason','approver','issued_utc','expires_utc');$actual=@($Approval.PSObject.Properties.Name);if(@($required|?{$_ -notin $actual}).Count -or @($actual|?{$_ -notin $required}).Count){throw 'approval schema mismatch'}
 if($Approval.schema_version -ne 1 -or $Approval.baseline_revision -notmatch '^[0-9a-f]{40}$' -or $Approval.old_revision -notmatch '^[0-9a-f]{40}$' -or $Approval.new_revision -notmatch '^[0-9a-f]{40}$' -or $Approval.new_tree -notmatch '^[0-9a-f]{40}$' -or $Approval.candidate_tree_digest -notmatch '^[0-9a-f]{64}$'){throw 'approval hex/schema invalid'}
 ...
 if($Approval.baseline_revision -ne $OldRevision -or $Approval.old_revision -ne $OldRevision -or $Approval.new_revision -ne $NewRevision -or $Approval.new_tree -ne $NewTree -or $Approval.candidate_tree_digest -ne $CandidateTreeDigest -or $Approval.workflow_run_id -ne $WorkflowRunId -or $Approval.nonce -ne $Nonce){throw 'approval identity mismatch'}
