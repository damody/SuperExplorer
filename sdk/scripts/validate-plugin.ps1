...
param(
    ...
    [string]$TemplateManifestSha256,
    [string]$ExpectedResolvedManifestSha256
)
...
if (-not (Test-Path -LiteralPath (Join-Path $localCargoRegistry 'cache') -PathType Container) -or -not (Test-Path -LiteralPath (Join-Path $localCargoRegistry 'index') -PathType Container)) {
    throw 'the local Cargo registry cache is unavailable; install the locked crates before running this offline validation'
}
...
$reportTemplateManifestSha256 = $null
try {
    ...
    if ([bool]$TemplateManifestSha256 -ne [bool]$ExpectedResolvedManifestSha256) { throw 'template and resolved manifest digests must be supplied together' }
    $reportTemplateManifestSha256 = if ($TemplateManifestSha256) { $TemplateManifestSha256 } else { [string]$templateMaterialization.template_manifest_sha256 }
    if ($reportTemplateManifestSha256 -notmatch '^[0-9a-f]{64}$') { throw 'template manifest digest must be a lowercase SHA-256 digest' }
    if ($ExpectedResolvedManifestSha256) {
        if ($ExpectedResolvedManifestSha256 -notmatch '^[0-9a-f]{64}$' -or $ExpectedResolvedManifestSha256 -ne [string]$templateMaterialization.resolved_manifest_sha256) { throw 'resolved manifest digest does not match the validated snapshot' }
    }
    Pop-Location
    $sdkPushed = $false
    ...
    foreach ($path in @($temporaryTarget,$temporarySnapshot)) {
...
try {
    ...
    $parsedReport | Add-Member -NotePropertyName inputs -NotePropertyValue ([ordered]@{
        consumer_tree_sha256 = $consumerTreeDigest
        template_manifest_sha256 = [string]$reportTemplateManifestSha256
        resolved_manifest_sha256 = [string]$templateMaterialization.resolved_manifest_sha256
        template_materialized = [bool]$templateMaterialization.materialized
    }) -Force
