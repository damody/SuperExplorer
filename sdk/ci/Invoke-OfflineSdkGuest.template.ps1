...
try {
    ...
    Invoke-Command -Session $session -ScriptBlock { param($root,$n) Set-Location $root; $env:SUPEREXPLORER_SENTINEL_NONCE=$n; powershell -NoProfile -File tests\toolchain-contract.ps1; if($LASTEXITCODE){exit $LASTEXITCODE}; powershell -NoProfile -File tests\network-isolation-sentinel.ps1; if($LASTEXITCODE){exit $LASTEXITCODE}; powershell -NoProfile -File tests\offline-host-plugin-contract.ps1; if($LASTEXITCODE){exit $LASTEXITCODE}; $routes=@(Get-NetRoute -DestinationPrefix '0.0.0.0/0','::/0' -ErrorAction SilentlyContinue); if($routes.Count){exit 9} } -ArgumentList 'C:\OfflineRun\sdk',$nonce -ErrorAction Stop
    $after=(Get-VMNetworkAdapter -VMName $guest).Count; $routes=Invoke-Command -Session $session { @(Get-NetRoute -DestinationPrefix '0.0.0.0/0','::/0' -ErrorAction SilentlyContinue).Count }; if($after -ne 0 -or $routes -ne 0){throw 'guest network isolation check failed'}
