...
4. Run `package-plugin.ps1`; it never rebuilds, revalidates the manifest and build hashes, creates a fixed-order/fixed-time `.sepack`, reopens and hashes every entry, and stages the package, checksum, and report before publishing them together. The `.sepack` is the final completion marker and a publish failure rolls back sidecars.
