...
impl BatchColumnProviderImplementationV1 for Provider {
    fn run(&self, context: BatchColumnContextV1) -> JobTerminalV1 {
        ...
        let outcome = service.query(LockOwnerQueryRequestV1 {
            items: context.items.iter().map(|item| item.item).collect::<Vec<_>>().into(),
        ...
        for item in &context.items {
            let owners = outcome.owners.iter().filter(|owner| owner.item == item.item).collect::<Vec<_>>();
            let result = if owners.is_empty() && outcome.status == LockOwnerQueryStatusV1::UNAVAILABLE {
                PluginItemResultV1::absent(PluginItemOutcomeV1::UNAVAILABLE)
            } else if owners.is_empty() {
                PluginItemResultV1::value(
                    PluginValueV1::structured_canonical_json(b"{\"count\":0,\"details\":\"\",\"names\":\"\"}".to_vec()).unwrap(),
                    ROption::RSome(StableSortValueV1::unsigned(0)),
                )
            } else {
                let names = owners.iter().map(|owner| owner.display_name.as_str()).collect::<Vec<_>>().join(", ");
                let details = owners.iter().map(|owner| {
                    format!("{} (PID {})", owner.display_name, owner.process_id)
                }).collect::<Vec<_>>().join("; ");
                let json = format!("{{\"count\":{},\"details\":\"{}\",\"names\":\"{}\"}}", owners.len(), escaped(&details), escaped(&names));
                match PluginValueV1::structured_canonical_json(json.into_bytes()) {
                    Ok(value) => PluginItemResultV1::value(value, ROption::RSome(StableSortValueV1::unsigned(owners.len() as u64))),
        ...
        if submitted.status.into_raw() == 1 { JobTerminalV1::COMPLETED } else { JobTerminalV1::PLUGIN_ERROR }
    }
}
...
impl VisualColumnImplementationV1 for Provider {
    ...
    fn render(&self, context: CellRenderContextV1) -> CellRenderPlanV1 {
        ...
        let field = |name: &str| text.split(&format!("\"{name}\":\"")).nth(1).and_then(|tail| tail.split('"').next()).unwrap_or("");
        let names = if field("names").is_empty() { field("language") } else { field("names") };
        CellRenderPlanV1 {
...
impl ExtensionRegistrarImplementationV1 for Registrar {
    ...
    fn register(&self, request: explorer_extension_api::RegistrarRequestV1) -> RegistrarOutputResultV1 {
        if request.abi_schema != ABI_SCHEMA_V1 {
            return RResult::RErr(AbiErrorV1::new(AbiErrorCodeV1::SCHEMA_MISMATCH, ROOT_MODULE_CONTRACT_ID_V1, request.abi_schema.into_raw()));
        }
        RResult::ROk(RegistrarOutputV1 {
            outcome: RegistrationOutcomeV1::accepted(2),
            contributions: vec![
                RegisteredContributionV1 {
                    feature_id: FEATURE_ID.into(), contribution_id: COLUMN_ID.into(),
                    kind: RegisteredContributionKindV1::COLUMN,
                    required_capabilities: vec!["abi".into(), "filesystem.read".into(), "lock-owner.query".into()].into(),
                    interface_id: INTERFACE_ID,
                    expected_sort: ROption::RSome(explorer_extension_api::StableSortValueKindV1::U64),
                    opaque_contract: ROption::RNone,
                    renderer_contribution_id: ROption::RSome(RENDERER_ID.into()),
                    provider: ROption::RNone, visual_column: ROption::RNone,
                    size_map_view: ROption::RNone,
                    batch_column_provider: ROption::RSome(BatchColumnProviderObjectV1::new(Provider)),
                },
                RegisteredContributionV1 {
                    feature_id: FEATURE_ID.into(), contribution_id: RENDERER_ID.into(),
                    kind: RegisteredContributionKindV1::GPUI_RENDERER,
                    required_capabilities: vec!["abi".into()].into(), interface_id: INTERFACE_ID,
                    expected_sort: ROption::RNone, opaque_contract: ROption::RNone,
                    renderer_contribution_id: ROption::RNone, provider: ROption::RNone,
                    visual_column: ROption::RSome(explorer_extension_ui_api::VisualColumnObjectV1::new(Provider)),
                    size_map_view: ROption::RNone, batch_column_provider: ROption::RNone,
                }
            ].into(),
        })
    }
...
pub fn plugin_root() -> ExtensionRootModuleV1_Ref {
    ExtensionRootModuleV1::new::<Registrar>(
        PluginMetadataV1 { plugin_id: PLUGIN_ID, primary_interface_id: INTERFACE_ID },
        ROption::RNone,
    ).leak_into_prefix()
}
...
#[cfg(test)]
mod tests {
    use super::LockOwnerQueryRequestV1;
    #[test]
    fn request_is_bounded_and_generation_scoped() {
        ...
        let request = LockOwnerQueryRequestV1 {
            items: Vec::new().into(), item_generation: 7, location_generation: 9,
            deadline_millis: 250, reserved: 0,
        };
        assert_eq!((request.item_generation, request.location_generation), (7, 9));
    }
}
