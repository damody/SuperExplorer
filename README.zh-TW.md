...
- 資料夾選項 → 一般可單選搜尋引擎：Everything、MFT 或檔案列舉。不支援的選項會停用並顯示原因，搜尋不會默默改用其他引擎。
...
- 資料夾選項視窗可設定搜尋引擎、檢視偏好、語言、擴充啟用、快取預算與範圍化工作階段重設。
...
- 搜尋只使用資料夾選項中選取的引擎。Everything 需要已包裝的 SDK 與 IPC；MFT 需要本機 NTFS 與 SuperExplorer MFT 索引；檔案列舉需要本機資料夾。遠端位置無法使用這三個引擎。
