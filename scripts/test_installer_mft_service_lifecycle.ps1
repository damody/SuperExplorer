...
Require-Text 'IntCmp $2 10 service_stop_timeout service_stop_retry service_stop_timeout' 'five-second graceful stop deadline'
Require-Text '"$SYSDIR\taskkill.exe" /F /FI "SERVICES eq SuperExplorerMft"' 'service-scoped recovery termination after graceful timeout'
Require-Text 'service_stop_timeout_final:' 'bounded recovery STOPPED wait timeout'
...
Require-UninstallText '${UnStrStr} $3 $1 "1061"' 'concurrent service-transition race handling'
...
Require-UninstallText 'IntCmp $2 10 un.service_stop_timeout un.service_stop_retry un.service_stop_timeout' 'five-second graceful stop deadline'
Require-UninstallText 'un.service_stop_timeout_final:' 'bounded recovery STOPPED wait timeout'
