...
local root = assert(arg[1], "test root is required")
...
local unicode_cwd = expect(lfs.currentdir(), "cannot read Unicode current directory")
expect(unicode_cwd:sub(-#directory_name) == directory_name, "currentdir did not return UTF-8")
...
print(string.format("LuaFileSystem UTF-8 smoke passed: version=%s path_bytes=%d", lfs._VERSION, #deep))
