...
- [x] 1.1.1 定義 Folder/Bookmark 節點、穩定 ID、parent 與 sibling order。
- [x] 1.1.2 實作建立、改名、移動及同層重排 mutation。
- [x] 1.1.3 實作無子節點 folder 刪除與 mutation rollback。
- [x] 1.1.4 實作非空 folder descendant-count 與確認後遞迴刪除 mutation。
- [x] 1.1.5 對循環 parent、無效 parent、重複 ID 和非法 order 加入恢復測試。
...
- [x] 1.2.1 實作扁平 `Bookmarks` JSON 至根節點的向後相容解碼。
- [x] 1.2.2 將新樹接入 session snapshot、hydration 與 last-known-good 恢復。
- [x] 1.2.3 測試 legacy ID/name/target/Lua source/order 無損升級。
- [x] 1.2.4 測試耐久寫入失敗時完整回復 pre-mutation tree。
...
- [x] 3.1.1 將 tree 資料夾投影到 favourites navigation，含展開/收合 state 與 UIA roles。
- [x] 3.1.2 實作 root 空白區／資料夾節點的右鍵命令及 action dispatch。
- [x] 3.1.3 實作新增 root/subfolder 和重新命名對話框。
- [x] 3.1.4 實作 non-empty folder descendant-count 確認與持久化 rollback。
- [x] 3.1.5 新增 keyboard、右鍵與不影響 filesystem 的聚焦 tests。
...
- [x] 3.2.1 擴充 bookmark draft 以記錄 parent、模式與可選移除。
- [x] 3.2.2 實作可存取 root/folder tree picker 及預選目的地。
- [x] 3.2.3 將星號從立即 toggle 改為 create-or-edit draft，並維持 selection independence。
- [x] 3.2.4 將 selected file/folder 加入書籤改接 destination draft。
- [x] 3.2.5 在既有 bookmark editor 提供確認後移除操作。
- [x] 3.2.6 測試 root、子資料夾、existing edit/remove、non-filesystem disabled 與 persistence failure。
...
- [x] 4.1.1 更新 toolbar 與 overflow 以顯示 root folder submenu 和 sibling order。
- [x] 4.1.2 更新 manager 以顯示 nested folders、move destination 與 context commands。
- [x] 4.1.3 維持 folder/file activation、missing target notice 與 Lua background dispatch。
- [x] 4.1.4 執行 Lua host-boundary 與 `.explorer.lua` negative regression tests。
...
- [x] 5.1.1 執行 explorer-model、explorer-ui、explorer-app、explorer-automation 聚焦測試。
- [x] 5.1.2 執行 formatter、clippy 與工作區相關編譯檢查。
