...
Every registrar contribution SHALL reference one manifest feature ID, and each feature SHALL declare all capabilities used by its callbacks. Undeclared, duplicate or capability-exceeding contributions SHALL reject the package.
...
Rust DLLs SHALL load only during application startup and remain resident until process exit. Runtime disabling SHALL gate new dispatch, cancel jobs, remove contributions and bounded-drain callbacks without unloading the DLL. Installing, replacing, removing or enabling an unloaded DLL SHALL require restart.
