...
- [x] 1.1.1 Record the exact dirty-worktree baseline, relevant file hashes, GPUI revision, and current Folder Options source/test locations.
- [x] 1.1.2 Add a failing Rust assertion proving Open Folder Options still enters the Explorer overlay instead of requesting a dedicated window.
...
- [x] 1.1.4 Create append-only evidence records for the baseline commands, expected failures, actual results, hashes, and timestamps.
...
- [x] 1.2.2 Define draft snapshot, applied baseline revision, dirty state, and typed apply success/failure results.
- [x] 1.2.3 Add contract tests for a single live instance, stale-handle replacement, retry after creation failure, and idempotent close.
- [x] 1.2.4 Add reducer tests for Apply baseline replacement, OK-after-success, and Cancel/Escape/title-close discard equivalence.
...
- [x] 2.1.3 Route successful Apply through existing persistence and broadcast paths with a monotonic applied-settings revision.
- [x] 2.1.4 Keep dirty drafts stable across external setting broadcasts while allowing clean drafts to adopt the latest snapshot.
- [x] 2.1.5 Emit structured, non-fatal diagnostics for create, activate, validation, and persistence failures.
- [x] 2.1.6 Run and record focused controller/state tests, including two live Explorer windows receiving one Apply snapshot.
...
- [x] 2.2.4 Route Tab/Shift+Tab, page focus entry, Enter, Escape, and native title-close through the typed entity/controller transitions.
...
- [x] 2.2.6 Add render and focus tests proving the overlay is absent and all three pages/actions remain accessible at minimum size.
...
- [x] 3.1.3 Reuse or extract Explorer scrollbar geometry for thumb ratio, track paging, pointer capture, grab offset, and clamping.
...
- [x] 3.1.6 Add unit/render tests for independent offsets, resize clamping, fit/overflow states, and 100/125/150/200 percent DPI coordinate conversion.
...
- [x] 3.2.2 Terminate options scrollbar capture idempotently on mouse-up-outside, Escape, deactivation, capture loss, and native close.
- [x] 3.2.3 Add interaction tests that compare options, file-view, and navigation offsets before and after every supported scroll gesture.
...
- [x] 4.1.3 Automate wheel, track, thumb, Page Up/Down, Home/End, and per-page offset restoration while recording background Explorer offsets.
- [x] 4.1.4 Automate Apply, OK, Cancel, Escape, title-close, creation retry/stale replacement where injectable, and two-window setting broadcast.
- [x] 4.1.5 Run minimum-size and 100/125/150/200 percent DPI assertions or record evidence-backed `not-applicable` only when the runner cannot set a scale on the available display.
- [x] 4.1.6 Save screenshots, HWND/count, logical/physical bounds, offsets, action results, environment metadata, hashes, and terminal status in the evidence index.
...
- [x] 4.2.3 Build a requirement/scenario-to-task/gate/evidence traceability table and mark any failed, stale, or unexecuted leaf incomplete.
