...
系統 SHALL 在書籤工具列最左側固定提供單選檔案系統檔案或資料夾的星號切換，並允許使用者從右鍵選單加入書籤；系統 SHALL 提供建立 Lua 書籤的工具列 `+` 操作與完整書籤管理員。管理員 MUST 支援改名、依型別編輯 payload、重排與刪除。

#### Scenario: 以星號切換選取項目的書籤狀態

- **WHEN** 使用者單選檔案系統檔案或資料夾並按下空心星號
- **THEN** 系統 SHALL 立即加入該項目的型別化書籤、持久化變更並在工具列最左側顯示實心星號
- **WHEN** 使用者再次按下實心星號
- **THEN** 系統 SHALL 立即移除該書籤、持久化變更並恢復空心星號
