...
這避免工具列與管理員產生雙重排序或競態。星號是工具列第一個固定控制項，不參與書籤排序或 overflow，並維持 20 px 的醒目尺寸。它只投影目前分頁 history current location：當位置可解析為實體檔案系統資料夾時，建立 `BookmarkTarget::Folder` 並以型別化目標查找既有 bookmark ID；空心／實心星號分別執行加入／移除。檔案清單 selection 不參與星號狀態或目標計算，非檔案系統位置則停用。選取檔案或子資料夾的快速加入仍由右鍵選單提供；Lua 僅由 `+` 或管理員建立。

星號 action 與 state helper 使用「目前資料夾」命名，避免保留 `selected` 語意造成日後回歸。mutation、durable session 通知與持久化失敗 rollback 沿用既有書籤流程。
...
- [目前位置與選取項目混淆] → 星號狀態只讀取 active tab 的 current location，並以零選取及切換選取的測試證明目標不變。
