...
#define STAT_FUNC lfs_win32_stat
...
#ifdef _WIN32
#define lfs_mkdir lfs_win32_mkdir
#define lfs_chdir lfs_win32_chdir
#else
#define lfs_mkdir(path) (mkdir((path), \
    S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH))
#define lfs_chdir chdir
#endif
...
static int lfs_win32_lstat(const char *path, STAT_STRUCT * buffer)
{
  ...
  if (wide == NULL)
    return 1;
  if (GetFileAttributesExW(wide, GetFileExInfoStandard, &win32buffer)) {
    if (!(win32buffer.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT)) {
      int result = _wstati64(wide, buffer);
      free(wide);
      return result;
    }
...
{
#ifdef _WIN32
  char *path = lfs_win32_getcwd();
  if (path == NULL)
    return pusherror(L, "get_dir GetCurrentDirectoryW() failed");
...
static int lfs_lock_dir(lua_State * L)
{
  ...
  fd = CreateFileW(wide, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
static int make_link(lua_State * L)
{
  ...
  int is_dir = 0;
  wchar_t *wide_oldpath;
  wchar_t *wide_newpath;
  ...
  wide_oldpath = lfs_utf8_to_path(oldpath);
  wide_newpath = lfs_utf8_to_path(newpath);
  if (wide_oldpath == NULL || wide_newpath == NULL) {
    free(wide_oldpath);
    free(wide_newpath);
    return pusherror(L, "make_link UTF-8 conversion failed");
  }
  int result = symbolic ? CreateSymbolicLinkW(wide_newpath, wide_oldpath, is_dir)
      : CreateHardLinkW(wide_newpath, wide_oldpath, NULL);
  free(wide_oldpath);
  free(wide_newpath);
...
static int remove_dir(lua_State * L)
{
  const char *path = luaL_checkstring(L, 1);
#ifdef _WIN32
  return pushresult(L, lfs_win32_rmdir(path), NULL);
#else
...
#ifdef _WIN32
  if (d->hFile == NULL) {         /* first entry */
    if ((d->hFile = FindFirstFileW(d->pattern, &c_file)) == INVALID_HANDLE_VALUE) {
      lua_pushnil(L);
      lua_pushstring(L, strerror(errno));
      d->closed = 1;
      return 2;
    } else {
      char *name = lfs_utf16_to_utf8(c_file.cFileName);
      if (name == NULL)
        return pusherror(L, "directory entry UTF-8 conversion failed");
      lua_pushstring(L, name);
      free(name);
      return 1;
    }
  } else {                      /* next entry */
    if (!FindNextFileW(d->hFile, &c_file)) {
...
#ifdef _WIN32
  if (!d->closed && d->hFile && d->hFile != INVALID_HANDLE_VALUE) {
    FindClose(d->hFile);
  }
  free(d->pattern);
  d->pattern = NULL;
...
#ifdef _WIN32
  {
    size_t pattern_length = strlen(path) + 3;
    char *utf8_pattern = (char *)malloc(pattern_length);
    if (utf8_pattern == NULL)
      return luaL_error(L, "out of memory opening directory: %s", path);
    snprintf(utf8_pattern, pattern_length, "%s/*", path);
    d->hFile = NULL;
    d->pattern = lfs_utf8_to_path(utf8_pattern);
    free(utf8_pattern);
    if (d->pattern == NULL)
      return luaL_error(L, "cannot convert directory path to UTF-16: %s", path);
  }
#else
  d->dir = opendir(path);
  if (d->dir == NULL)
...
#ifdef _WIN32
  return pushresult(L, lfs_win32_utime(file, buf), NULL);
#else
...
static int push_link_target(lua_State * L)
{
  const char *file = luaL_checkstring(L, 1);
#ifdef _WIN32
  wchar_t *wide_file = lfs_utf8_to_path(file);
  wchar_t *target = NULL;
  DWORD tsize = 0, size = 256;
  int ok = 0;
  HANDLE h;
  if (wide_file == NULL)
    return 0;
  h = CreateFileW(wide_file, GENERIC_READ,
  ...
  while (!ok) {
    wchar_t *target2 = (wchar_t *)realloc(target, (size_t)size * sizeof(wchar_t));
    if (!target2) {             /* failed to allocate */
      break;
    }
    target = target2;
    tsize = GetFinalPathNameByHandleW(h, target, size, FILE_NAME_OPENED);
    if (tsize == 0) {
      break;
    }
    if (tsize < size) {
      if (tsize > 4 && wcsncmp(target, L"\\\\?\\", 4) == 0) {
        memmove(target, target + 4, (tsize - 3) * sizeof(wchar_t));
        tsize -= 4;
      }
      ok = 1;
      break;
