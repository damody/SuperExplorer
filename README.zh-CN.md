...
- 文件夹选项 → 常规可单选搜索引擎：Everything、MFT 或文件枚举。不支持的选项会停用并显示原因，搜索不会默默改用其他引擎。
...
- 搜索只使用文件夹选项中选择的引擎。Everything 需要已打包的 SDK 与 IPC；MFT 需要本地 NTFS 与 SuperExplorer MFT 索引；文件枚举需要本地文件夹。远程位置无法使用这三个引擎。
